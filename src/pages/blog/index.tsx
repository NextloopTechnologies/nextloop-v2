  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col flex-1 gap-2.5 px-1  py-4 ">
        ...
        <button className="self-start mt-auto text-[11px] font-bold tracking-[0.08em] uppercase text-orange-500  transition-colors duration-200 bg-transparent border-none p-0 cursor-pointer"
          onClick={(e) => {
            e.stopPropagation();
            onClick();
          }}
    ...
    <Layout>
      ...
      {error ? (
        ...
        <div className="bg-white min-h-screen px-4 flex flex-col justify-center items-center md:px-8 xl:px-24 py-12 ">
          <div className='relative bg-black w-11/12 h-[75vh] cursor-pointer'
          ...
          <h2 className={`${palette.fontSize.heading2.mobile} md:text-4xl uppercase font-bold text-black mt-20 `}>
            ALL <span className="text-orange-500">BLOGS</span>
          </h2>

          <div className="grid grid-cols-1 mt-12 md:grid-cols-2 xl:grid-cols-3 gap-6 items-stretch">
            {data?.map((blog) => (
              <BlogCard
                key={blog.id}
                blog={blog}
