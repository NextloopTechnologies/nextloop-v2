...
const leftInventoryCardData = [
  ...
  { title: 'easy online booking & cancellation', Icon: Calendar, bg: '#1D1D1D' },
];

const rightInventoryCardData = [
  { title: 'clear planning & costing of your packages', Icon: ClipboardCheck, bg: '#1D1D1D' },
  { title: 'secure and multiple payment gateway integration', Icon: CreditCard, bg: '#FA8145' },
  { title: 'excellent customer support', Icon: Headphones, bg: '#1D1D1D' },
  { title: 'utmost comfort level', Icon: Coffee, bg: '#FA8145' },
  { title: 'appealing both desktop & mobile user', Icon: Monitor, bg: '#1D1D1D' },
];
...
const InventoryCard: React.FC<{
  ...
  return (
    ...
    >
      <div className={`flex ${align === 'left' ? 'flex-row' : 'flex-row-reverse'} w-full p-2`}>
        <div className='flex items-center px-4'>
          <Icon size={24} className={align === 'left' ? 'mr-3' : 'ml-3'} />
          <p className={`text-left ${align === 'left' ? 'pl-3' : 'pr-3'}`}>{title}</p>
        </div>
      </div>
    </div>
  );
      ...
      <div className='flex flex-col items-center justify-center xl:flex-row md:mx-10 mt-[50px] md:gap-4'>
        <div className='w-full md:w-auto'>
          {leftInventoryCardData.map(({ title, bg, Icon }, idx) => (
            <InventoryCard key={idx} title={title} Icon={Icon} bg={bg} align='left' />
          ))}
        </div>
